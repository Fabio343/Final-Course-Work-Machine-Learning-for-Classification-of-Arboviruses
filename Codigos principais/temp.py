...
def plot_roc_curve(y_true, y_score,nome,figsize=(6,6)):
...
dados_analises1=pd.read_excel('bio_marcador.xlsx',sep=';',error_bad_lines=False)

variaveis_dummy_2=[]
for k in ['exam1_techinic','exam1_resultado_desc']:
  base_dummy_novas= pd.get_dummies(dados_analises[k],prefix=k)
  dados_analises=dados_analises.merge(base_dummy_novas,left_index=True,right_index=True)
 
  for i in base_dummy_novas.columns:
    variaveis_dummy_2.append(i)

dados_analises.columns

for index, row in dados_analises.iterrows():
    if row['exam1_techinic_ELISA IgG'] == row['exam1_resultado_desc_Reactive']:
        dados_analises.loc[index,'IGG_reativo'] =1
    else:
        dados_analises.loc[index,'IGG_reativo'] =0
    
    if row['exam1_techinic_ELISA IgM '] == row['exam1_resultado_desc_Reactive']:
        dados_analises.loc[index,'IGM_reativo'] =1
    else:
        dados_analises.loc[index,'IGM_reativo'] =0
        
    if row['exam1_techinic_PCR'] == row['exam1_resultado_desc_Reactive']:
        dados_analises.loc[index,'PCR_reativo'] =1
    else:
        dados_analises.loc[index,'PCR_reativo'] =0


for index, row in dados_analises.iterrows():
    if row['exam1_techinic_ELISA IgG'] != row['exam1_resultado_desc_Reactive']:
        dados_analises.loc[index,'IGG_nao_reativo'] =1
    else:
        dados_analises.loc[index,'IGG_nao_reativo'] =0
    
    if row['exam1_techinic_ELISA IgM '] != row['exam1_resultado_desc_Reactive']:
        dados_analises.loc[index,'IGM_nao_reativo'] =1
    else:
        dados_analises.loc[index,'IGM_nao_reativo'] =0
        
    if row['exam1_techinic_PCR'] != row['exam1_resultado_desc_Reactive']:
        dados_analises.loc[index,'PCR_nao_reativo'] =1
    else:
        dados_analises.loc[index,'PCR_nao_reativo'] =0

base_unificada5=pd.read_excel('var_nov_tcc.xlsx',sep=';',error_bad_lines=False)
#base_unificada5['localidade']=0
base_unificada5=base_unificada5.merge(dados_analises1[['Subject_ID','MARCADOR_SuPAR']],how='left',on=['Subject_ID'])
...
base_unificada5=base_unificada5.merge(base_total[['Subject_ID','CLASSIFICACAO FINAL','HT 1','HB 1','PLAQ 1','TGO 1','TGP 1','localidade']],how='left',on=['Subject_ID'])
base_unificada5=base_unificada5.merge(dados_analises[['Subject_ID','IGG_nao_reativo','IGM_nao_reativo','PCR_nao_reativo','IGG_reativo','IGM_reativo','PCR_reativo']],how='left',on=['Subject_ID'])

base_unificada5.columns
...
base_unificada5['HT 1'].replace({"-": 0}, inplace=True)
base_unificada5['HB 1'].replace({"-": 0}, inplace=True)
#base_unificada5['LEUCO 1'].replace({"-": 0}, inplace=True)
base_unificada5['PLAQ 1'].replace({"-": 0}, inplace=True)
base_unificada5['TGO 1'].replace({"-": 0}, inplace=True)
base_unificada5['TGP 1'].replace({"-": 0}, inplace=True)
...
amostra_paci=amostra_paci[['localidade','fl_severidade','HT 1','HB 1','PLAQ 1','TGO 1','TGP 1','IGG_nao_reativo','IGM_nao_reativo','PCR_nao_reativo','IGG_reativo','IGM_reativo','PCR_reativo']]
...
X = amostra_paci[variaveis_features]
Y = amostra_paci['fl_severidade']
...
print(removed_feats)
print(len(removed_feats))
print(len(amostra_paci.columns))
        
for L in removed_feats: 
    if L=='localidade':
        continue
    else:
...
x_train,x_test=amostra_paci[(amostra_paci['localidade']==1.0)],amostra_paci[(amostra_paci['localidade']==0.0)]
#x_train,x_test,y_train,y_test=train_test_split(amostra_paci,amostra_paci['fl_severidade'],test_size=0.3,random_state=0)
y_train,y_test=amostra_paci['fl_severidade'][(amostra_paci['localidade']==1.0)],amostra_paci['fl_severidade'][(amostra_paci['localidade']==0.0)]
...
x_train.drop('localidade',axis=1,inplace=True)
x_test.drop('localidade',axis=1,inplace=True)
amostra_paci.drop('localidade',axis=1,inplace=True)
...
pickle_lista.close()
#removendo variaveis coonstante em 99% pois não agregaram para o modelo 
amostra_paci.drop(vars_const, axis=1, inplace=True)
x_train.drop(vars_const, axis=1, inplace=True)
x_train_1.drop(vars_const, axis=1, inplace=True)
x_test.drop(vars_const, axis=1, inplace=True)
